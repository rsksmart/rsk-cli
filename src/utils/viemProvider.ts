...
class ViemProvider {
  ...
  private async decryptPrivateKey(): Promise<{ account: ReturnType<typeof privateKeyToAccount> }> {
    ...
    if (!encryptedPrivateKey || !iv) {
      throw new Error('No valid encrypted private key found in the saved wallet.');
    }
    ...
    try {
      ...
      throw new Error('Failed to decrypt the private key. Please check your password.');
    }
  }
}
